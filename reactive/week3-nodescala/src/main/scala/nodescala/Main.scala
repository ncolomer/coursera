...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val userInterrupted: Future[String] = Future.userInput("Type something... ")
      .continue({case Success(m) => s"You entered... $m" case _ => "You entered... nothing!"})

    // TO IMPLEMENT
    // 3. create a future that completes after 20 seconds
    //    and continues with a `"Server timeout!"` message
    val timeOut: Future[String] = Future.delay(20 seconds).continue(u => "Server timeout!")

    // TO IMPLEMENT
    // 4. create a future that completes when either 10 seconds elapse
    //    or the user enters some text and presses ENTER
    val terminationRequested: Future[String] = Future.any[String](List(userInterrupted, timeOut))
